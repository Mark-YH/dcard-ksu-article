...
try:
    ...
    prefix='https://www.dcard.tw/_api/forums/ksu/posts?popular=false&before='
    articleID = '225579686' #目前KSU板最新的文章
    rs= requests.session()
    while True:
        ...
        if str(res)=='<Response [200]>':
            ...
            for i in range(jData.__len__()):
                ...
                forumName = jData[i]['forumName'] # 崑山科大板
                forumAlias = jData[i]['forumAlias'] # 崑山科大板的代號
                ...
                INSERT INTO `{}`(`articleID`, `forumName`, `forumAlias`, `title`, `school`, `department`, `gender`, `createdAt`, `updatedAt`,
                 ...
                 VALUES ('{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}','{}')
                '''.format(tableName,articleID, forumName, forumAlias, title, school,department,gender,createdAt,updatedAt,commentCount,likeCount,excerpt,path)
                c.execute(sql)
                conn.commit()
                print('success')
                print('-----')
